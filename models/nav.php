...
class Nav extends Conexion
{
    ...
    public function quantity($table)
    {
        $query = "SELECT COUNT(*) AS 'Cantidad' FROM modulos;";
        return $this->ConsultaSimple($query);
    }
    ...
    public function getall($arrayForm): array
    {
        ...
        $query = "SELECT * FROM modulos AS md LEFT JOIN menu ON md.id_modulo = menu.id_modulo LEFT JOIN  submenu AS smenu ON menu.id_menu =  smenu.id_menu RIGHT JOIN asignaciones ON smenu.id_submenu = asignaciones.subMenu_id WHERE usuario_id = {$arrayForm} ORDER BY md.id_modulo, menu.nombre_menu, smenu.orden_submenu;";
        return $this->ConsultaSimple($query);
    }
