...
const requestInsert = async (doc) => {
  ...
  const returned = await fetch("./../controllers/controllerLabor.php", { method: "POST", body });
...
const makeRequests = async (doc) => {
  ...
  console.log(data);
  body.append("data", JSON.stringify(data));
  const returned = await fetch("./../controllers/controllerLaborList.php", { method: "POST", body });
  const result = await returned.json();//await JSON.parse(returned);
